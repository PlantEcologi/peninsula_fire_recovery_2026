...
mdatwd <- "data/"
mname <- "Landsat_samples_peninsulaMay2026" #model name for file naming
...
# Landsat ndvi from LANDSAT/COMPOSITES/C02/T1_L2_32DAY_NDVI
NDVI <- rast("data/Landsat_NDVI_1984_2026.tif") #new NDVI data
#crs(NDVI) <- "EPSG:3857" #original projection of the data
NAflag(NDVI) <- -9999
NDVI <- trim(NDVI)
NDVIdates <- read.csv("data/Landsat_NDVI_dates_1984_2026.csv") # Load dates
...
writeRaster(NDVI[[1]], paste0(mdatwd,"Landsat_NDVI_input_grid.tif"), overwrite=T)

# # Extract NDVI time series to plots and inspect...
# ndvi_plots <- as.data.frame(t(extract(NDVI, pts, method = "simple", na.rm = T))[-1,])
# ndvi_plots$Date <- as.Date(substr(rownames(ndvi_plots),1,8), format = "%Y%m%d")
# ndvi_plots <- pivot_longer(ndvi_plots, cols = -Date, names_to = "Site", values_to = "NDVI")  
#   # filter(NDVI > 0) |> 
#   # group_by(Site) |> 
#   # filter(n() >= 50) |> 
#   # ungroup()
...
sta <- c(dem, slope, tpi, aspect, northness, eastness, vegtype) |> 
  setNames(c("elevation", "slope", "tpi", "aspect", "northness", "eastness", "vegtype"))

rm(list = c("dem", "aspect", "northness", "eastness", "slope", "tpi"))
...
#vegetation age
rfi <- rast("data/Landsat_veldage2022.tif") #new fire age data
...
###########################################################
###Extract raster data to df using points 
###########################################################

# Create 450 "background" points for extraction by randomly sampling the NDVI grid within the vegtype polygons

set.seed(2026) #for reproducibility

# Sample 1000
ndvisum <- sum(NDVI>0, na.rm = T) #gives variation in NDVI missingness and can be used to filter for NA

bgpts <- spatSample(c(sta, ndvisum), size = 1000, method = "spread", exact = T, na.rm = T, xy=TRUE) |> 
  filter(vegtype %in% c("dune", "sandstone", "sand")) |>  #drop beach, granite and wetland veg types with few observations
  filter(sum > 200) #drop points with many NA NDVI values

# Keep all dune and sand samples and randomly sample sandstone to get a total of 450 points
bgpts <- bind_rows(filter(bgpts, vegtype %in% c("dune", "sand")), 
                   slice_sample(filter(bgpts, vegtype == "sandstone"), n = 450 - nrow(filter(bgpts, vegtype %in% c("dune", "sand")))))

# Reformat to merge with plot points
bgpts <- bgpts |>
    mutate(plot = paste(x, y, sep = "_")) |>
    st_as_sf(coords = c("x", "y"), crs = crs(pts)) |>
    select(plot) |>
    mutate(background = 1) 
...
ndat <- extract(NDVI, pts, method = "simple", na.rm = F) |> 
  ...
  mutate(UIJ = pts$plot) |>
  select(!c("ID")) |>
  pivot_longer(cols = -c("UIJ"), names_to = "Date", values_to = "value") |>
  separate_wider_delim(Date, names = c("Date", "variable"), delim = "_") |>
...
envars <- c("elevation", "slope", "tpi", "northness") 
...
# require(GGally)
# ggpairs(cov[,1:7]) # ggpairs(as.data.frame(env_full))
...
save(env,file=paste(mdatwd,mname, Sys.Date(),"_envdata.Rdata",sep="")) #save env for analysing results
